...
def run(tool_path: Path, input_fasta: Path, output_dir: Path, batch_size: int):
    ...
    try:
        ...
        tree_file = input_fasta.with_suffix(input_fasta.suffix + ".tree")  # Input file with .tree suffix
        if tree_file.exists():
            tree_output_file = output_dir / f"{input_fasta.stem}.tree"
            shutil.move(tree_file, tree_output_file)
            logging.info(f"✅ Guide tree saved to {tree_output_file}")
        else:
            logging.warning("⚠️ No guide tree file generated by MAFFT.")
